...
func (a *App) SetupRoutes() *http.ServeMux {
	...
	mux.Handle("POST /api/agreements/{id}/contract/generate",
		middleware.JWTAuth(a.Cfg.JWTSecret, http.HandlerFunc(agreementHandler.GenerateContract)))
	...
	mux.Handle("GET /api/notifications",
		middleware.JWTAuth(a.Cfg.JWTSecret, http.HandlerFunc(notificationHandler.GetAll)))
	mux.Handle("GET /api/notifications/{id}",
		middleware.JWTAuth(a.Cfg.JWTSecret, http.HandlerFunc(notificationHandler.GetByID)))
	mux.Handle("PATCH /api/notifications/{id}/read",
